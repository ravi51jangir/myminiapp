...
const HOMEPAGE_3  = dynamic(() => Promise.resolve(({ children }) =>{
    ...
    const { contractData, buyTokens, isConfirming } = usePresaleContract();
  ...
  const formatKValue = (value) => {
    ...
    const calculateProgress = () => {
        ...
        return Number((contractData.totalRaised * BigInt(100)) / contractData.hardCap);
    };
  ...
  <button
    ...
    onClick={handleBuy}
    disabled={!isConnected || isConfirming || !amount || parseFloat(amount) <= 0}
    style={{
      cursor: isConnected && !isConfirming && amount ? 'pointer' : 'not-allowed',
      //opacity: !isConnected || isConfirming || !amount ? 0.6 : 1,
    }}
  >
    {isConfirming ? 'Processing...' : 'Buy NEUTON'}
  </button>
