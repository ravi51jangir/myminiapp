...
const HOMEPAGE_3 = dynamic(
  () =>
    Promise.resolve(({ children }) => {
      ...
      const { contractData, buyTokens, approveUSDT, isConfirming, currentAllowance } = usePresaleContract();
      ...
      const handleBuy = async () => {
        ...
        console.log("Handle buy triggered", isConnected);
        if (!isConnected || !amount) return;
        
        try {
            console.log("Attempting purchase with amount:", amount);
            
            // Request account access first
            await window.ethereum.request({ method: 'eth_requestAccounts' });
            
            // First check and handle USDT approval
            if (!currentAllowance || currentAllowance < parseEther(amount)) {
                setIsApproving(true);
                try {
        ...
        <div className={styles.homePage3}>
          ...
          <div className={styles.buynewtonrectangle}>
            ...
            <button
              ...
              style={{
                cursor:
                  isConnected && !isConfirming && !isApproving && amount
                    ? "pointer"
                    : "not-allowed",
              }}
            >
